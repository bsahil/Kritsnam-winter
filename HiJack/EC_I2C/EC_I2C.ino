...
//#define address 112              //default I2C ID number for EZO EC Circuit.
...
void loop() {                   //the main loop.
 ...
 int addresses[10] ;
  ...
  if (serial_event == true) {   //if a command was sent to the EC circuit.
    ...
    if (computerdata[0] == 'c' || computerdata[0] == 'r')delay_time = 2000; //if a command has been sent to calibrate or take a reading we wait 2000ms so that the circuit has enough time to take the reading.
    else delay_time = 300;                                                  //if any other command has been sent we wait only 300ms.

for(i=0;i<nDevices;i++){

    Wire.beginTransmission(addresses[i]);            //call the circuit by its ID number.
    ...
    if (strcmp(computerdata, "sleep") != 0) {   //if the command that has been sent is NOT the sleep command, wait the correct amount of time and request data.
      ...
      Wire.requestFrom(addresses[i], 48, 1);          //call the circuit and request 48 bytes (this is more than we need)
